...
export async function loadLeaderboard(targetId) {
  ...
  wrap.innerHTML = guilds.map((g, i) => {
    ...
    const typeChips = types.map(tp => `<span class="badge badge-${typeClass(tp)}" style="font-size:.65rem;">${typeIcon(tp)} ${tp}</span>`).join('');
...
export async function loadAdminGuilds() {
  const wrap = document.getElementById('admin-guilds-list'); if (!wrap) return;
  ...
  wrap.innerHTML = guilds.map(g => {
    ...
    const guildTeams = (allTeams || []).filter(t => t.guild_id === g.id);
    const types = [...new Set(guildTeams.map(t => t.type).filter(Boolean))];
    const typeChipsHtml = types.map(tp => `<span class="badge badge-${typeClass(tp)}">${typeIcon(tp)} ${tp}</span>`).join('');
...
export function adminSetXP(guildId) {
  const val = prompt('ตั้งค่า XP ใหม่:'); if (val === null) return;
  const xp = parseInt(val); if (isNaN(xp) || xp < 0) return toast('ค่าไม่ถูกต้อง', 'var(--red)');
  db.from('guilds').update({ xp }).eq('id', guildId).then(() => { toast('อัปเดต XP แล้ว'); loadAdminGuilds(); });
}
...
export async function confirmEditGuild() {
  ...
  closeModal('modal-edit-guild'); toast('💾 บันทึกแล้ว'); loadAdminGuilds();
}

export function openDeleteGuild(id, name) {
  state.pendingDeleteGuildId = id; state.pendingDeleteGuildName = name;
...
export async function renderManageMembersList(guildId) {
  ...
  if (!members || members.length === 0) { wrap.innerHTML = '<div class="empty" style="padding:16px;">ยังไม่มีสมาชิก</div>'; if (countEl) countEl.textContent = '0'; return; }
  if (countEl) countEl.textContent = members.length;
  wrap.innerHTML = members.map(m => `<div class="manage-member-row"><div class="manage-member-avatar" style="background:${m.role === 'leader' ? '#d97706' : 'var(--blue2)'};">${m.name[0].toUpperCase()}</div><div class="manage-member-name">${m.name}</div><span class="badge ${m.role === 'leader' ? 'badge-leader' : 'badge-member'}">${m.role === 'leader' ? '👑' : ''}</span><div style="display:flex;gap:5px;"><button class="btn btn-ghost btn-sm" onclick="openEditMember(${m.id},'${escStr(m.name)}','${m.role}')">✏️</button><button class="btn btn-red btn-sm" onclick="adminRemoveMember(${m.id},'${escStr(m.name)}')">🗑️</button></div></div>`).join('');
}
...
export async function adminAddMember() {
  ...
  await renderManageMembersList(state.managingGuildId); loadAdminGuilds();
}
...
export async function confirmEditMember() {
  ...
  if (error) return toast('เกิดข้อผิดพลาด: ' + error.message, 'var(--red)');
  closeModal('modal-edit-member'); toast('💾 บันทึกแล้ว');
  await renderManageMembersList(state.managingGuildId); loadAdminGuilds();
}
...
export async function loadAdminQuestList() {
  const wrap = document.getElementById('admin-quest-list'); if (!wrap) return;
  ...
  wrap.innerHTML = quests.map(q => `<div class="quest-item"><div class="quest-icon">${q.icon || '⚔️'}</div><div style="flex:1;"><div class="quest-title">${q.title}</div><div class="quest-desc">${q.description || ''}</div><div class="quest-xp">⭐ ${q.xp_reward || q.points || 0} XP · ${q.guild_id ? `กิลด์: ${q.guilds?.name}` : 'ทุกกิลด์'}</div></div><button class="btn btn-red btn-sm" onclick="deleteQuest(${q.id})">ลบ</button></div>`).join('');
}
...
export async function deleteQuest(id) {
  ...
  toast('ลบเควสแล้ว', 'var(--muted)'); loadAdminQuestList();
}
...
export async function loadAdminSubmissions() {
  ...
  if (!subs || subs.length === 0) { wrap.innerHTML = '<div class="empty">ยังไม่มีการส่งงาน</div>'; if (countEl) countEl.style.display = 'none'; return; }
  ...
  wrap.innerHTML = subs.map(s => {
    ...
    const statusLabel = s.status === 'approved' ? '✅ อนุมัติ' : s.status === 'rejected' ? '❌ ปฏิเสธ' : '⏳ รอ Approve';
    const teamInfo = s.teams ? ` · 👥 ${s.teams.name}${s.teams.type ? ` <span class="badge badge-${typeClass(s.teams.type)}" style="font-size:.65rem;">${typeIcon(s.teams.type)} ${s.teams.type}</span>` : ''}` : '';
...
export function viewBigImage(src) { document.getElementById('modal-big-img').src = src; document.getElementById('modal-view-img').classList.add('open'); }
...
export async function approveSubmission(subId, guildId, xpReward, questTitle, memberName) {
  ...
  loadAdminSubmissions(); loadAdminGuilds();
}
...
export async function rejectSubmission(subId, memberName) {
  ...
  toast('ปฏิเสธแล้ว', 'var(--red)'); loadAdminSubmissions();
}

export function updateAdminTeamCountChip() {
  const chip = document.getElementById('admin-team-count-chip'); if (!chip) return;
...
export function addTeamMemberInput() {
  ...
  if (shared.teamMembersBuilder.length >= MAX_TEAM_MEMBERS) { toast(`สูงสุด ${MAX_TEAM_MEMBERS} คน`, 'var(--red)'); return; }
  if (shared.teamMembersBuilder.includes(name)) { toast('ชื่อซ้ำ', 'var(--amber)'); return; }
  shared.teamMembersBuilder.push(name); input.value = '';
  renderTeamMembersBuilder(); updateAdminTeamCountChip();
}
...
export function renderTeamMembersBuilder() {
  const wrap = document.getElementById('team-members-builder'); if (!wrap) return;
  if (shared.teamMembersBuilder.length === 0) { wrap.innerHTML = '<span class="muted" style="font-size:.8rem;">ยังไม่มีสมาชิก</span>'; return; }
  wrap.innerHTML = shared.teamMembersBuilder.map(n => `<span class="badge badge-member" style="cursor:pointer;" onclick="removeTeamMember('${escStr(n)}')">${n} ✕</span>`).join('');
}
...
export async function loadAdminTeamsTab() {
  ...
  shared.teamMembersBuilder = []; renderTeamMembersBuilder(); updateAdminTeamCountChip();
  loadAdminAllTeams();
}
...
export async function loadAdminAllTeams() {
  const wrap = document.getElementById('admin-teams-list'); if (!wrap) return;
...
export async function deleteTeam(teamId) {
  ...
  toast('ลบกลุ่มแล้ว', 'var(--muted)'); loadAdminAllTeams();
}
...
export async function loadAdminRedeemRequests() {
  ...
  if (!reqs || reqs.length === 0) { wrap.innerHTML = '<div class="empty">ยังไม่มีคำขอ</div>'; if (countEl) countEl.style.display = 'none'; return; }
...
export function openApproveRedeem(reqId, cardCode, guildName, guildId) {
  ...
  document.getElementById('modal-approve-redeem-content').innerHTML = `<div class="notice notice-info">รหัสการ์ด: <strong>${cardCode}</strong></div><div style="margin-top:10px;">กิลด์: <strong>${guildName}</strong></div><div class="muted mt-s" style="font-size:.84rem;">ตรวจสอบรหัสใน Discord ก่อนอนุมัติ</div>`;
...
export async function confirmApproveRedeem(approve) {
  ...
  if (approve) {
    ...
    if (!card) {
      ...
      loadAdminRedeemRequests(); return;
    }
...
export async function adminCreateCard() {
  ...
  if (imageInput.files && imageInput.files[0]) {
    ...
    try { imageData = await new Promise((res, rej) => { const r = new FileReader(); r.onload = () => res(r.result); r.onerror = rej; r.readAsDataURL(file); }); } catch (e) { return toast('อ่านรูปผิดพลาด', 'var(--red)'); }
  }
...
export async function loadAdminAllCards() {
  const wrap = document.getElementById('admin-all-cards'); if (!wrap) return;
  const { data: cards } = await db.from('cards').select('*').order('created_at', { ascending: false });
  if (!cards || cards.length === 0) { wrap.innerHTML = '<div class="muted" style="grid-column:1/-1;">ยังไม่มีการ์ด</div>'; return; }
  const { data: gc } = await db.from('guild_cards').select('card_id,used,guild_id,guilds(name)');
  const gcMap = {}; (gc || []).forEach(g => { gcMap[g.card_id] = g; });
  wrap.innerHTML = cards.map(c => {
    const assign = gcMap[c.id];
    const isUsed = assign?.used;
    const statusHtml = isUsed ? `<span class="badge badge-rejected" style="margin-top:6px;">✔️ ใช้แล้ว (${assign.guilds?.name || '?'})</span>` : assign ? `<span class="badge badge-approved" style="margin-top:6px;">🎯 กับ ${assign.guilds?.name || '?'}</span>` : `<span class="badge badge-pending" style="margin-top:6px;">📦 ว่าง</span>`;
    const deleteBtn = isUsed ? `<div class="muted" style="font-size:.72rem;margin-top:6px;">🔒 ใช้แล้ว ลบไม่ได้</div>` : `<button class="btn btn-red btn-sm btn-full" style="margin-top:8px;" onclick="askDeleteCard(${c.id},'${escStr(c.name)}')">🗑️ ลบ</button>`;
    if (c.image_data) {
      return `<div class="card-item" style="padding:0;overflow:hidden;"><div style="width:100%;aspect-ratio:3/4;overflow:hidden;"><img src="${c.image_data}" style="width:100%;height:100%;object-fit:cover;"/></div><div style="padding:10px;"><div class="card-name">${c.name}</div><div class="muted" style="font-size:.72rem;">🔑 ${c.card_code}</div>${statusHtml}${deleteBtn}</div></div>`;
    }
    return `<div class="card-item"><div class="card-emoji">${c.icon || '🃏'}</div><div class="card-name">${c.name}</div><div class="card-effect">${c.effect || ''}</div><div class="muted" style="font-size:.72rem;">🔑 ${c.card_code || '-'}</div>${statusHtml}${deleteBtn}</div>`;
  }).join('');
}

export function askDeleteCard(cardId, cardName) {
  state.pendingDeleteCardId = cardId;
  document.getElementById('modal-delete-card-desc').textContent = `ลบการ์ด "${cardName}" ออกจากระบบถาวร?`;
...
export async function confirmDeleteCard() {
  ...
  if (gc?.used) { toast('การ์ดนี้ถูกใช้แล้ว ลบไม่ได้', 'var(--red)'); return; }
  ...
  toast('ลบการ์ดแล้ว', 'var(--muted)'); loadAdminAllCards();
}

export async function loadAdminCompTab() { loadAdminCompList(); }

export async function loadAdminCompList() {
  const wrap = document.getElementById('admin-comp-list'); if (!wrap) return;
...
export async function deleteCompetition(compId) {
  ...
  toast('ลบแล้ว', 'var(--muted)'); loadAdminCompList();
}

export async function loadAdminLog() {
  const wrap = document.getElementById('console-log'); if (!wrap) return;
